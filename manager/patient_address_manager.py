...
class PatientAddressManager:

    def __init__(self) -> None:
        self.address_dao = AddressDao()

    def update_patient_address(self, address_id, patient_id, request_payload):
        address = self.get_patient_address(address_id, patient_id)
    ...
    def get_patient_address(self, address_id, patient_id):
        if not address_id and not address_id.strip():
            raise InvalidPayloadException(ErrorStatusCode.PATIENT_ADDRESS_RETRIEVAL_FAILED, "address id is invalid.")

        address = self.address_dao.get_address_for_id(address_id)

        if address and address.user_id != patient_id:
            raise InvalidPayloadException(ErrorStatusCode.PATIENT_ADDRESS_RETRIEVAL_FAILED, "No address found.")
